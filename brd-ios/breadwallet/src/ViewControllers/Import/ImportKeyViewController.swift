...
class ImportKeyViewController: UIViewController, Subscriber {
    ...
    init(wallet: Wallet, initialQRCode: QRCode? = nil) {
        ...
        assert(wallet.currency.isBitcoin || wallet.currency.isBitcoinCash || wallet.currency.isBitcoinSV || wallet.currency.isLitecoin, "Importing only supports BTC, BSV, BCH, or LTC")
