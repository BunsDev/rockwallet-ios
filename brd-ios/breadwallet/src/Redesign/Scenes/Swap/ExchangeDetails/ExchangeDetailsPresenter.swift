...
final class ExchangeDetailsPresenter: NSObject, Presenter, ExchangeDetailsActionResponses {
    ...
    func presentData(actionResponse: FetchModels.Get.ActionResponse) {
        ...
        let formattedCurrencyAmountDestination = ExchangeFormatter.current.string(for: detail.destination.currencyAmount) ?? ""
    ...
    private func prepareOrderViewModel(_ detail: SwapDetail, for type: TransactionType) -> BuyOrderViewModel {
        ...
        let currencyFormat = "%@ %@"
        let rate = String(format: "1 %@ = %@ %@", detail.destination.currency, ExchangeNumberFormatter().string(for: 1 / detail.rate) ?? "",
                          currencyCode)
